...
# Vertical override — Montserrat at a smaller size so chunks fit one line,
# narrower margins so captions don't span full width, MarginV=60 places them
# ~20% above the bottom edge.
SUB_FORCE_STYLE_VERTICAL = (
    "FontName=Montserrat,FontSize=10,Bold=1,"
    "PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BackColour=&H80000000,"
    "BorderStyle=1,Outline=0.5,Shadow=1,"
    "Alignment=2,MarginV=60,MarginL=30,MarginR=30"
)
...
def extract_segment(
    ...
    Vertical mode outputs 1080x1920 (9:16). Per-segment zoom can be applied:
      - zoom_mode="snap": constant zoom level for the whole segment
      - zoom_mode="smooth": animates from zoom_from at t=0 to zoom at t=duration
    """
    ...
    if vertical:
        ...
        # Scale source so width matches target, then center-crop to target height
        scale = f"scale={out_w}:-2,crop={out_w}:{out_h}:(in_w-{out_w})/2:(in_h-{out_h})/2"
    else:
        if draft:
    ...
    # Per-segment zoom: applied AFTER the scale-to-target step so the zoom
    # math works in target pixel space regardless of source resolution.
    # Implementation: center-crop a smaller region of the already-scaled frame,
    # then scale that crop back up to (out_w, out_h). Larger zoom -> smaller crop.
    zoom_filter = ""
    if zoom_mode == "smooth" and abs(zoom - zoom_from) > 0.001 and duration > 0.01:
        # Use the `zoompan` filter — purpose-built for animated zoom with stable
        # output dimensions per frame (avoids encoder crashes from time-varying
        # scale output sizes). zoompan centers its crop by default.
        # z(on) = z_from + (z_target - z_from) * on / total_frames
        total_frames = max(1, int(round(duration * 24)))
        z_expr = f"{zoom_from}+({zoom}-{zoom_from})*on/{total_frames}"
        zoom_filter = (
            f"zoompan=z='{z_expr}':d=1:s={out_w}x{out_h}:fps=24"
        )
    elif abs(zoom - 1.0) > 0.001:
        # Snap (constant) zoom — fixed crop
        cw = out_w / zoom
        ch = out_h / zoom
        zoom_filter = (
            f"crop=w={cw:.2f}:h={ch:.2f}:x='(in_w-out_w)/2':y='(in_h-out_h)/2',"
            f"scale={out_w}:{out_h}"
        )
...
def extract_all_segments(
    ...
    Tracks the previous segment's zoom per source clip so smooth zooms can
    animate from the prior level rather than from 1.0. This produces the
    "punch-in" effect that hides jump cuts within the same A-roll clip.
    """
    ...
    # Per-source-clip running zoom level, used as zoom_from for smooth zooms
    prev_zoom_per_source: dict[str, float] = {}
    ...
    for i, r in enumerate(ranges):
        ...
        zoom_mode = r.get("zoom_mode", "snap")
        zoom_from = prev_zoom_per_source.get(src_name, 1.0)

        note = r.get("beat") or r.get("note") or ""
        zoom_note = ""
        if abs(zoom - 1.0) > 0.001 or zoom_mode == "smooth":
            zoom_note = f" [zoom {zoom_from:.2f}→{zoom:.2f} {zoom_mode}]"
        print(f"  [{i:02d}] {src_name}  {start:7.2f}-{end:7.2f}  ({duration:5.2f}s)  {note}{zoom_note}")
        if is_auto:
            print(f"        grade: {seg_filter or '(none)'}")
        extract_segment(
            src_path, start, duration, seg_filter, out_path,
            preview=preview, draft=draft, vertical=vertical,
            zoom=zoom, zoom_mode=zoom_mode, zoom_from=zoom_from,
        )
        seg_paths.append(out_path)

        # Update the running zoom level for this source (the zoom at end-of-segment)
        prev_zoom_per_source[src_name] = zoom
...
def _titlecase(s: str) -> str:
    """Capitalize first character of each whitespace-separated word.

    Unlike str.title(), this preserves mid-word casing in apostrophe contractions
    ("I'm", "It's", "You'd") and brand spellings ("PayPal", "iPhone").
    """
    out = []
    for w in s.split():
        if not w:
...
def build_master_srt(
    ...
    - chunk_size word chunks (break on any punctuation in between)
    - case: "upper" (default) | "title" | "none"
    ...
    for r in edl["ranges"]:
        ...
        for w in words_in_seg:
            ...
            if not text:
                continue
            current.append(w)
            # Break if the current text ends in punctuation or we hit chunk_size words
        ...
        for chunk in chunks:
            ...
            # case == "none" → leave Scribe casing untouched
...
def build_final_composite(
    ...
    if has_subs:
        # On Windows, ffmpeg's filtergraph parser chokes on backslashes inside
        # quoted filter values (libavfilter treats them as escape characters
        # even within single quotes). Use forward slashes universally; ffmpeg
        # accepts them on Windows.
        subs_abs = (
        ...
        style = SUB_FORCE_STYLE_VERTICAL if vertical else SUB_FORCE_STYLE
        filter_parts.append(
...
def main() -> None:
    ...
    ap.add_argument(
        "--vertical",
        action="store_true",
        help="Output 9:16 (1080x1920). Also switches captions to narrower, "
             "non-uppercase, 5-word style ~20%% above bottom.",
    )
    ap.add_argument(
        "--caption-words",
        type=int,
        default=None,
        help="Words per caption chunk (default: 2 for horizontal, 5 for --vertical).",
    )
    ap.add_argument(
        "--caption-uppercase",
        action="store_true",
        help="Force UPPERCASE captions (default: on for horizontal, off for --vertical).",
    )
    ap.add_argument(
        "--caption-case",
        choices=["upper", "title", "none"],
        default=None,
        help="Caption case transform. Defaults: 'upper' for horizontal, 'title' for --vertical.",
    )
    args = ap.parse_args()
    ...
    # Caption style choice (vertical implies narrower + title-case + 5-word chunks)
    if args.caption_words is not None:
        chunk_size = args.caption_words
    else:
        chunk_size = 5 if args.vertical else 2
    if args.caption_case is not None:
        case = args.caption_case
    elif args.caption_uppercase:
        case = "upper"
    else:
        case = "title" if args.vertical else "upper"
    ...
    if not args.no_subtitles:
        if args.build_subtitles:
            ...
            build_master_srt(edl, edit_dir, subs_path, chunk_size=chunk_size, case=case)
    ...
    if args.no_loudnorm:
        ...
        build_final_composite(base_path, overlays, subs_path, out_path, edit_dir, vertical=args.vertical)
    else:
        # Composite to a temp file, then run loudnorm → final output
        tmp_composite = out_path.with_suffix(".prenorm.mp4")
        build_final_composite(base_path, overlays, subs_path, tmp_composite, edit_dir, vertical=args.vertical)
