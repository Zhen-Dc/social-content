...
Read `SOP.md` before using this skill independently. It defines the editing workflow, output directory contract, helper commands, QA checks, and handoff rules.
...
Helpers (`helpers/transcribe.py`, `helpers/render.py`, etc.) live alongside this SKILL.md. Resolve their paths relative to the directory containing this file — the skill is typically symlinked at `~/.Codex/skills/video-use/` or `~/.codex/skills/video-use/`.
